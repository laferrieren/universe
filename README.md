...
#### Table of Contents
* [Universe Function](#universe-function)
...
To publish a package to Universe, fork this repo and open a Pull Request. A set of automated builds will be ran against
the Pull Request to ensure the modifications made in the PR conform to the schema of the repository and each of the
files corresponding to your package.  See [Creating a Package](#creating-a-package) for details.
...
packages are coordinated for the version of the package and are required to be adhered to.
...
and ran in a location accessible by the DC/OS Cluster. See [Universe Server](#universe-server) for details on
...
Currently, a package can specify two value for `.packagingVersion` either `2.0` or `3.0`, which version is declared
...
installation time by passing an options file to the DC/OS CLI or by setting config value through the
...
Describes how to install the package's pip CLI. This document represents the format of a Pip requirements file where
each element in the array is a line in the requirements file.
...
The new `.cli` property allows for a package to configure "native" CLI sub commands for several platforms and
architectures.
...
5. Submit a pull request against the `version-3.x` branch with your changes. Every pull request opened will have a set
   of automated verifications ran against it. These automated verification are reported against the pull request using
...
In order for Universe to be consumed by DC/OS the build process need to be ran to create the Universe Server.

### Universe Server

Universe Server is a new component introduce along side `packagingVersion` `3.0`. In order for Universe to be able to
provide packages for many version of DC/OS at the same time, it is necessary for a server to be responsible for serving
the correct set of packages to a cluster based on the clusters version.

All Pull Requests opened for Universe and the `version-3.x` branch will have their docker image build and published
...
will provide ONLY those packages which can be ran on the cluster.

For example:
A DC/OS 1.6.1 Cluster will only receive package with a `minDcosReleaseVersion` less than or equal to ()`<=`) `1.6.1`
